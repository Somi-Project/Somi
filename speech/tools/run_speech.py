...
from speech.brain.agent_bridge import ask_agent, init_agent_bridge
...
async def _main(args):
    ...
    backend = (args.tts_backend or "piper").lower()
    if backend in {"pocket", "pocket_server"}:
        backend = "piper"
    tts = get_tts_engine({"TTS_ENGINE": backend})
...
def parse_args():
    ...
    p.add_argument("--tts-backend", default=os.getenv("SOMI_TTS_BACKEND", "piper"))
    p.add_argument("--echo-policy", default=os.getenv("SOMI_ECHO_POLICY", "tier0"))
...
if __name__ == "__main__":
    args = parse_args()
    logger.info("Starting speech user_id=%s os_profile=%s tts_backend=%s echo_policy=%s", args.user_id, args.os_profile, args.tts_backend, args.echo_policy)
