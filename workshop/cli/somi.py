...
def _load_optional_handler(import_path, handler_name, feature_name):
    try:
        module = __import__(import_path, fromlist=[handler_name])
    except ModuleNotFoundError as exc:
        missing = exc.name or "dependency"
        raise click.ClickException(
            f"{feature_name} support is unavailable because '{missing}' is not installed."
        ) from exc
...
async def gencookies():
    TwitterHandler = _load_optional_handler(
        "workshop.integrations.twitter", "TwitterHandler", "Twitter"
    )
...
async def telegram(name, use_studies):
    ...
    if not agent_key:
        ...
        return
    display_name = agent_key.replace("Name: ", "")
    TelegramHandler = _load_optional_handler(
        "workshop.integrations.telegram", "TelegramHandler", "Telegram"
    )
