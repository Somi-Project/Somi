...
def parse_skill_md(skill_dir: Path) -> SkillDoc:
    ...
    if isinstance(metadata, dict):
        direct_runtime = metadata.get("runtime")
        if isinstance(direct_runtime, dict):
            runtime_meta = direct_runtime
        else:
            # Compatibility fallback: pick the first metadata block with known runtime keys.
            for value in metadata.values():
                if not isinstance(value, dict):
                    continue
                if any(k in value for k in ("skillKey", "requires", "primaryEnv", "homepage", "emoji")):
                    runtime_meta = value
                    break
    runtime_meta = runtime_meta if isinstance(runtime_meta, dict) else {}

    skill_key = str(runtime_meta.get("skillKey") or name)
    homepage = frontmatter.get("homepage") or runtime_meta.get("homepage")
    emoji = runtime_meta.get("emoji")
