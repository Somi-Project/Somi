...
from workshop.integrations.twitter.state_store import StateStore
from workshop.integrations.twitter.policy import should_noop
from workshop.integrations.twitter.selectors import (
...
class TwitterHandler:
    def __init__(self, character_name=None, use_studies=False):
        ...
        self.toolbox_runtime = InternalToolRuntime()
...
"""
    ...
    async def generate_tweet(self):
        ...
        thought_starters = {
            ...
            "neutral": ["Just thinking...", "Random thought...", "Todayâ€™s vibe is..."]
        }
        ...
        user_prompt = (
            ...
            f"- If stuck, return 'Canâ€™t think of anything to tweet right now.'"
        )
        messages = [
        ...
        while attempts < 3:
            try:
                ...
                attempts += 1
        fallback_tweets = {
            "empathetic": "My mindâ€™s wandering today. Iâ€™ll share a thought soon.",
            "playful": "Brainâ€™s on a playful break. Back with a quip in a bit!",
            "analytical": "Dataâ€™s not sparking ideas yet. Iâ€™ll tweet soon.",
            "neutral": "Canâ€™t think of anything to tweet right now."
        }
        return fallback_tweets.get(self.behaviors[0], "Canâ€™t think of anything to tweet right now.")
    ...
    async def generate_reply(self, tweet_text, username, user_id):
        ...
        if should_search:
            ...
            try:
                tool_out = await asyncio.to_thread(
                    self.toolbox_runtime.run,
                    "web.intelligence",
                    {
                        "query": search_query,
                        "tool_veto": False,
                        "reason": "twitter_reply",
                        "signals": {"intent": "general"},
                        "route_hint": "twitter_reply",
                    },
                    {"source": "twitter", "approved": True, "user_id": user_id},
                )
                if bool((tool_out or {}).get("ok", True)):
                    formatted_results = str((tool_out or {}).get("formatted") or "")[:1500]
                else:
                    formatted_results = ""
            except Exception as exc:
                logger.warning(f"Web tool lookup failed: {exc}")
                formatted_results = ""
        ...
        while attempts < 3:
                    ...
                    for word in normalized_content.split():
                        if re.match(r'^\d+(\.\d+)?$', word):
                            ...
                            if normalized_word not in normalized_results and word not in current_time.lower():
                                ...
                                content = f"Sorry, I canâ€™t verify that info right now!"
                                break
                return content
            if not content:
            ...
            attempts += 1
        fallback_replies = {
            "empathetic": "Youâ€™ve got me thinking! Iâ€™ll reply with something soon.",
            "playful": "That oneâ€™s got me stumped! Iâ€™ll come back with a quip.",
    ...
    async def analyze_image(self, image_path, caption, user_id):
        ...
        prompt = (
            ...
            f"Youâ€™ve received an image with caption: '{caption}'.\n"
        ...
        try:
            ...
            for mention in mentions:
                ...
                if len(reply_message) < min_chars:
                    reply_message = (reply_message + " Appreciate your pointâ€”curious how youâ€™d apply this in practice?").strip()
