"""
ReligionDomain â€” free, no-auth retrieval for religious studies, theology,
sacred texts overviews, scriptures, exegesis, comparative religion, etc.

Now includes:
- Academic sources for scholarly papers
- Wikipedia for overviews
- Wikisource for public domain sacred text excerpts
- Sacred Texts Archive (sacred-texts.com) for full classic translations of holy books
All sources public/no-key
"""
...
import httpx
from bs4 import BeautifulSoup  # BeautifulSoup is lightweight and safe for this
...
def _looks_like_religion(q: str) -> bool:
    ql = (q or "").lower()
    triggers = [
        "religion", "theology", "god", "allah", "jesus", "muhammad", "buddha", "krishna",
        "bible", "quran", "koran", "torah", "talmud", "vedas", "bhagavad gita", "upanishads",
        "book of mormon", "guru granth sahib", "tao te ching", "dhammapada",
        "verse", "chapter", "surah", "psalm", "exegesis", "hermeneutics", "interpretation",
        "faith", "belief", "doctrine", "denomination", "sect", "church", "mosque", "temple",
        "prayer", "ritual", "sacrament", "pilgrimage", "holy", "sacred", "scripture",
        "comparative religion", "interfaith", "atheism", "agnosticism", "spirituality",
    ]
...
class ReligionDomain:
    ...
    async def search(self, query: str, *, retries: int = 2, backoff_factor: float = 0.5) -> List[Dict[str, Any]]:
        ...
        try:
            async with httpx.AsyncClient(
                ...
                headers={"User-Agent": "SomiReligion/1.0"},
                ...
                tasks: List[Any] = [
                    ...
                    self._search_wikisource(client, q, retries=retries, backoff_factor=backoff_factor),
                    self._search_sacredtexts(client, q, retries=retries, backoff_factor=backoff_factor),
                ...
                if not merged:
                    return [self._sentinel(
                        ...
                        "No relevant religious/theological data found in academic or sacred text sources.",
                        query=q,
                    )]

                rel = _looks_like_religion(q)
                for r in merged:
                    src = str(r.get("source") or "")
                    if src in ("wikisource", "sacredtexts"):
                        r["intent_alignment"] = 1.3 if rel else 0.98  # Strong boost for primary texts
                    elif src == "wikipedia":
                        r["intent_alignment"] = 1.1 if rel else 0.90
                    elif src in ("semanticscholar", "openalex"):
                        r["intent_alignment"] = 1.0 if rel else 0.85
                    elif src == "arxiv":
                        r["intent_alignment"] = 0.90 if rel else 0.80
                    elif src == "crossref":
                        r["intent_alignment"] = 0.75
                    else:
                        r["intent_alignment"] = 0.70

                logger.info(f"ReligionDomain returned {len(merged)} results for '{q}'")
                return merged

        except Exception as e:
            logger.warning(f"ReligionDomain search failed: {type(e).__name__}: {e}")
            return [self._sentinel(
                "Science search unavailable",
                "Religious sources unreachable (network error).",
                query=q,
            )]
    ...
    # -----------------------------
    # Semantic Scholar â€” theological research
    # -----------------------------
    ...
    # -----------------------------
    # OpenAlex â€” theological studies
    # -----------------------------
    ...
    # -----------------------------
    # Crossref â€” theological metadata
    # -----------------------------
    ...
    # -----------------------------
    # arXiv â€” emerging religious studies
    # -----------------------------
    ...
    # -----------------------------
    # Wikipedia â€” religion overviews, figures, texts
    # -----------------------------
    async def _search_wikipedia(
        ...
        *,
        ...
        async with self._src_sem:
            search_q = quote_plus(q + " religion OR theology OR scripture OR god OR prophet")
            ...
            for page in pages[:4]:
                ...
                spans.insert(0, f"Religious overview: {title}")

                r = pack_result(
                    title=f"Religion: {title}",
                    url=wiki_url,
    ...
    # -----------------------------
    # Wikisource â€” sacred texts (public domain full texts)
    # -----------------------------
    async def _search_wikisource(
        self,
        client: httpx.AsyncClient,
        q: str,
        *,
        ...
        async with self._src_sem:
            search_q = quote_plus(q)
            search_url = f"https://en.wikisource.org/w/api.php?action=query&list=search&srsearch={search_q}&srlimit=10&format=json"
            search_js = await self._get_json(client, search_url, retries=retries, backoff_factor=backoff_factor)
            if not search_js:
                return []
            ...
            for page in pages[:4]:
                ...
                extract_url = (
                    f"https://en.wikisource.org/w/api.php?action=query&prop=extracts&exintro&explaintext&titles={quote_plus(title)}&format=json"
                )
                extract_js = await self._get_json(client, extract_url, retries=retries, backoff_factor=backoff_factor)
                if not extract_js:
                    continue
                ...
                wikisource_url = f"https://en.wikisource.org/wiki/{quote_plus(title.replace(' ', '_'))}"

                spans = make_spans_from_text(content)[:8]
                spans.insert(0, f"Sacred text excerpt: {title}")

                r = pack_result(
                    title=f"Religious text: {title}",
                    url=wikisource_url,
                    description=safe_trim(content, 1200),
                    source="wikisource",
                    domain=self.DOMAIN,
    ...
    # -----------------------------
    # Sacred Texts Archive â€” classic public domain translations
    # -----------------------------
    async def _search_sacredtexts(
        self,
        client: httpx.AsyncClient,
        q: str,
        *,
        ...
        async with self._src_sem:
            qq = quote_plus(q)
            search_url = f"https://www.sacred-texts.com/search/index.htm?q={qq}"
            txt = await self._get_text(client, search_url, retries=retries, backoff_factor=backoff_factor)
            if not txt:
                return []
            ...
            # Results are in <p><a href="...">Title</a> - description</p>
            results = soup.find_all("p", class_=False)[:6]  # Top results
            for res in results:
                a_tag = res.find("a")
                ...
                if not link.startswith("http"):
                    link = "https://www.sacred-texts.com" + link

                desc = res.get_text(strip=True)
                desc = desc.replace(title, "", 1).strip(" -â€“")
                desc = re.sub(r"\s+", " ", desc)

                spans = make_spans_from_text(desc)[:6]
                spans.insert(0, f"Sacred text: {title}")

                r = pack_result(
                    title=f"Holy book: {title}",
                    url=link,
