...
class RAGHandler:
    def __init__(self, storage_path="database/rag_data", nlist=100):
        ...
        self.index = None
        self.texts = []
        self.pdf_folder = Path("research") / "textbooks"
    ...
    async def study_pdfs(self):
        ...
        if not self.pdf_folder.exists():
            ...
            return
        for pdf_path in self.pdf_folder.rglob("*.pdf"):
            if "ingested" in pdf_path.parts:
                continue
            try:
    ...
    async def ingest_pdfs(self):
        """Unified textbook ingestion path + vector study."""
        try:
            from workshop.toolbox.stacks.research_core.science_ingest import ingest_folder
            books, chunks = ingest_folder(self.pdf_folder)
            logger.info(f"Textbook ingest completed: books_added={books}, chunks_added={chunks}")
        except Exception as e:
            logger.warning(f"Textbook ingest layer failed, continuing with RAG vector ingest: {e}")
