...
# Spoken/phrase â†’ canonical code (expand as needed)
SPOKEN_TO_CODE = {
    # Fiat
...
class Converter:
    ...
    async def _get_usd_value(self, asset: str, amount: float = 1.0) -> Optional[float]:
        ...
        logger.info(f"â†’ Resolving {amount} {asset} to USD")
        ...
        try:
            ...
            if res and res[0] and "price_usd" in res[0]:
                ...
                logger.info(f"   Crypto success â†’ {asset} = {v:.2f} USD")
                return amount * v
        ...
        if len(asset) == 3 and asset.isalpha():
            try:
                ...
                if res and res[0]:
                    ...
                    if v is not None:
                        logger.info(f"   Forex success â†’ {asset} = {v:.6f} USD")
                        return amount * v
            except asyncio.TimeoutError:
        ...
        try:
            ...
            if res and res[0]:
                v = self._extract_price(res[0])
                if v is not None:
                    logger.info(f"   Commodity/stock success â†’ {asset} = {v:.2f} USD")
                    return amount * v
        except asyncio.TimeoutError:
    ...
    async def convert(self, query: str) -> str:
        ...
        try:
            ...
            return f"{amt_str} {parsed.src.upper()} â‰ˆ {conv_str} {parsed.dst.upper()}"
