...
class ArtifactIntentDetector:
    ...
    def _action_items_evidence(self, text: str, text_raw: str) -> Dict[str, Any]:
        ...
        structure = []
        for marker, signal in [("action items:", "action_items_heading"), ("todo:", "todo_heading"), ("next steps:", "next_steps_heading"), ("assigned to:", "assigned_to_heading"), ("tareas:", "action_items_heading_es"), ("prÃ³ximos pasos:", "next_steps_heading_es"), ("proximos pasos:", "next_steps_heading_es")]:
            if marker in text:
                structure.append(signal)
    ...
    def _status_evidence(self, text: str) -> Dict[str, Any]:
        explicit = bool(re.search(r"\b(status update|standup update|weekly update|write a standup|actualizacion de estado|actualizaciÃ³n de estado)\b", text))
