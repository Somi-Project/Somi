...
from workshop.toolbox.stacks.ocr_core.exports import export_json, export_structured_excel
...
from workshop.toolbox.stacks.ocr_core.prompts import structured_stage1_prompt, structured_stage2_normalize_prompt
from workshop.toolbox.stacks.ocr_core.quality import (
    escalation_metadata,
...
from workshop.toolbox.stacks.ocr_core.schema import ensure_default_schema_migrated, load_schema, schema_field_names
...
def structured_ocr(req: OcrRequest) -> OcrResult:
    ...
    for _ in range(max_passes):
        ...
        try:
            ...
            parser_strategy = "entry_blocks"

        heuristic_records = extract_structured_candidates(stage1_text, schema)
        if heuristic_records:
            heuristic_coverage, _, _, heuristic_score, _ = score_structured(heuristic_records, schema, stage1_text, metrics)
            current_coverage, _, _, current_score, _ = score_structured(records, schema, stage1_text, metrics)
            if heuristic_coverage > current_coverage or (heuristic_coverage == current_coverage and heuristic_score > current_score):
                records = heuristic_records
    ...
    field_confidence = field_confidence_map(records, schema, parse_failures, stage1_text)
    table_shape_rows = table_shapes(records, schema)
    ...
    return OcrResult(
        ...
        quality=quality,
        provenance={"model": model, "stage2_model": stage2_model, "preprocessed": preprocessed, "parser_strategy": parser_strategy},
