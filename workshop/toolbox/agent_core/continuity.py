...
from workshop.toolbox.stacks.contracts_core.base import build_base
...
STATE_LEDGER_MAX_ITEMS = 14


def _ledger_default(user_id: str) -> Dict[str, Any]:
    return {
...
def _ledger_path(user_id: str, root_dir: str = "sessions/state_ledger") -> Path:
    safe = re.sub(r"[^a-zA-Z0-9_.-]", "_", str(user_id or "default_user"))[:120]
    return Path(root_dir) / f"{safe}.json"


def _norm_item(text: Any, *, max_len: int = 220) -> str:
...
def _dedupe_trim(items: List[str], *, max_items: int = STATE_LEDGER_MAX_ITEMS) -> List[str]:
    out: List[str] = []
    seen = set()
    for raw in items:
        item = _norm_item(raw)
...
def _extract_goal_candidates(user_text: str) -> List[str]:
    text = str(user_text or "").strip()
    lower = text.lower()
    out: List[str] = []

    m = re.search(r"\bmy goal is\s+(.+)$", text, flags=re.IGNORECASE)
    if m:
        out.append(m.group(1).strip())

    m = re.search(r"\bi (?:need|want) to\s+(.+)$", text, flags=re.IGNORECASE)
    if m and len(m.group(1).strip()) >= 6:
        out.append(m.group(1).strip())

    m = re.search(r"\bobjective\s*:\s*(.+)$", text, flags=re.IGNORECASE)
    if m:
        out.append(m.group(1).strip())
    ...
    return _dedupe_trim(out, max_items=4)


def _extract_decision_candidates(user_text: str, assistant_text: str) -> List[str]:
    out: List[str] = []
    for text in (str(user_text or ""), str(assistant_text or "")):
        m = re.search(r"\b(?:decision|we decided|decided to|let'?s go with)\b\s*[:\-]?\s*(.+)$", text, flags=re.IGNORECASE)
        if m:
            out.append(m.group(1).strip())

    for line in str(assistant_text or "").splitlines():
        line_s = line.strip()
        if line_s.lower().startswith("decision:"):
            out.append(line_s.split(":", 1)[-1].strip())

    return _dedupe_trim(out, max_items=4)


def _extract_open_loop_candidates(user_text: str, assistant_text: str) -> List[str]:
    out: List[str] = []
    u = str(user_text or "").strip()
    a = str(assistant_text or "").strip()

    if re.search(r"\b(continue|resume|follow up|next step|what'?s left|pending)\b", u, flags=re.IGNORECASE):
        out.append(u)

    for line in a.splitlines():
        ls = line.strip()
        if re.search(r"^(next|todo|pending|open loop)\s*[:\-]", ls, flags=re.IGNORECASE):
            out.append(ls.split(":", 1)[-1].strip() if ":" in ls else ls)

    return _dedupe_trim(out, max_items=6)


def _extract_unresolved_candidates(user_text: str, assistant_text: str) -> List[str]:
    unresolved = []
    u = str(user_text or "").strip()
    a = str(assistant_text or "").strip().lower()

    is_question = "?" in u or bool(re.search(r"\b(can you|could you|would you|how|why|what|when|where|which)\b", u.lower()))
    weak_answer = any(
        marker in a
        for marker in (
            "i'm not sure",
    ...
    if is_question and weak_answer:
        unresolved.append(u)

    return _dedupe_trim(unresolved, max_items=6)


def load_state_ledger(user_id: str, *, root_dir: str = "sessions/state_ledger") -> Dict[str, Any]:
    path = _ledger_path(user_id, root_dir=root_dir)
    if not path.exists():
        return _ledger_default(user_id)
    try:
    ...
    out = _ledger_default(user_id)
    out.update({
        "updated_at": str(raw.get("updated_at") or out["updated_at"]),
        "turn_count": int(raw.get("turn_count") or 0),
        "goals": _dedupe_trim(list(raw.get("goals") or [])),
        "decisions": _dedupe_trim(list(raw.get("decisions") or [])),
        "open_loops": _dedupe_trim(list(raw.get("open_loops") or [])),
        "unresolved_asks": _dedupe_trim(list(raw.get("unresolved_asks") or [])),
        "last_user_intent": _norm_item(raw.get("last_user_intent"), max_len=260),
        "last_assistant_commitments": _dedupe_trim(list(raw.get("last_assistant_commitments") or []), max_items=8),
    })
    return out


def save_state_ledger(user_id: str, ledger: Dict[str, Any], *, root_dir: str = "sessions/state_ledger") -> Dict[str, Any]:
    path = _ledger_path(user_id, root_dir=root_dir)
    path.parent.mkdir(parents=True, exist_ok=True)

    payload = _ledger_default(user_id)
    payload.update({
        "updated_at": str((ledger or {}).get("updated_at") or _now().isoformat()),
        "turn_count": int((ledger or {}).get("turn_count") or 0),
        "goals": _dedupe_trim(list((ledger or {}).get("goals") or [])),
        "decisions": _dedupe_trim(list((ledger or {}).get("decisions") or [])),
        "open_loops": _dedupe_trim(list((ledger or {}).get("open_loops") or [])),
        "unresolved_asks": _dedupe_trim(list((ledger or {}).get("unresolved_asks") or [])),
        "last_user_intent": _norm_item((ledger or {}).get("last_user_intent"), max_len=260),
        "last_assistant_commitments": _dedupe_trim(list((ledger or {}).get("last_assistant_commitments") or []), max_items=8),
    })
...
def update_state_ledger(
    ...
    *,
    ...
    max_items: int = STATE_LEDGER_MAX_ITEMS,
) -> Dict[str, Any]:
    out = dict(ledger or load_state_ledger(user_id))

    goals = list(out.get("goals") or []) + _extract_goal_candidates(user_text)
    decisions = list(out.get("decisions") or []) + _extract_decision_candidates(user_text, assistant_text)
    open_loops = list(out.get("open_loops") or []) + _extract_open_loop_candidates(user_text, assistant_text)
    unresolved = list(out.get("unresolved_asks") or []) + _extract_unresolved_candidates(user_text, assistant_text)

    out["goals"] = _dedupe_trim(goals, max_items=max_items)
    out["decisions"] = _dedupe_trim(decisions, max_items=max_items)
    out["open_loops"] = _dedupe_trim(open_loops, max_items=max_items)
    out["unresolved_asks"] = _dedupe_trim(unresolved, max_items=max_items)

    out["last_user_intent"] = _norm_item(user_text, max_len=260)
    commitments = []
    for line in str(assistant_text or "").splitlines():
        ls = line.strip()
        if re.search(r"\b(i will|i can|next|plan|todo|we should)\b", ls, flags=re.IGNORECASE):
            commitments.append(ls)
    out["last_assistant_commitments"] = _dedupe_trim(
        list(out.get("last_assistant_commitments") or []) + commitments,
        max_items=8,
    )

    out["turn_count"] = int(out.get("turn_count") or 0) + 1
    out["updated_at"] = _now().isoformat()
    return out


def render_state_ledger_block(ledger: Optional[Dict[str, Any]], *, max_items: int = 5) -> str:
    data = dict(ledger or {})
    goals = [str(x).strip() for x in list(data.get("goals") or []) if str(x).strip()][: max_items]
    decisions = [str(x).strip() for x in list(data.get("decisions") or []) if str(x).strip()][: max_items]
    loops = [str(x).strip() for x in list(data.get("open_loops") or []) if str(x).strip()][: max_items]
    unresolved = [str(x).strip() for x in list(data.get("unresolved_asks") or []) if str(x).strip()][: max_items]
    ...
    last_intent = str(data.get("last_user_intent") or "").strip()
    if last_intent:
        lines.append(f"- last_user_intent: {last_intent[:220]}")

    if goals:
        lines.append("- goals:")
        for g in goals:
            lines.append(f"  - {g[:220]}")
    ...
    if unresolved:
        ...
        for u in unresolved:
            lines.append(f"  - {u[:220]}")

    if not (goals or decisions or loops or unresolved):
        lines.append("- ledger_empty: true")

    lines.append("- instruction: maintain continuity across follow-ups and compacted history")
    return "\n".join(lines)
