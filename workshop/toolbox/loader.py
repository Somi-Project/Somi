...
class ToolLoader:
    ...
    def _verify(self, entry: dict) -> None:
        ...
        for file_path in Path(entry["path"]).rglob("*"):
            if not file_path.is_file():
                continue
            rel_path = file_path.relative_to(entry["path"])
            rel = rel_path.as_posix()
            if rel == ".sandbox_home" or rel.startswith(".sandbox_home/"):
                continue
    ...
    def propose_exec(
        ...
        return ExecutionTicket(
            ...
            cwd=entry["path"],
            backend=str(((entry.get("runtime") or {}).get("default_backend") or "local")),
            allowed_capabilities=list(entry.get("capabilities") or []),
    ...
    def execute_with_approval(
        ...
        request = BackendExecutionRequest(
            ...
            allow_network=bool(ticket.allow_network),
            allow_external_apps=bool(ticket.allow_external_apps),
            allow_delete=bool(ticket.allow_delete),
            read_write_paths=list(ticket.paths_rw or []),
            read_only_paths=list(ticket.paths_ro or []),
            sandbox_root=ticket.cwd,
            options=dict(ticket.backend_options or {}),
        )
        try:
        ...
        append_event(
            ticket.job_id,
            "execution ended",
            {"ticket_hash": th, "code": result.returncode, "backend": result.backend},
        )
