...
class WebSearchHandler:
    ...
    def _extract_ticker_candidates(self, query: str) -> List[str]:
        """
        Extract plausible ticker symbols from the ORIGINAL query text (case-sensitive).
        We avoid uppercasing the whole query because it turns ordinary words into fake tickers.
        """
        q = (query or "").strip()
        if not q:
            return []
        ...
        # Strong signals: ticker TSLA / symbol TSLA / ticker: TSLA
        for m in re.finditer(r"\b(?:ticker|symbol)\s*[:=]?\s*([A-Z]{1,5})\b", q, flags=re.IGNORECASE):
            strong.add(m.group(1).upper())

        # Weak-ish: ALL-CAPS token in ORIGINAL query
        tokens = re.findall(r"\b[A-Z]{2,5}\b", q)

        # Common false positives (extend as logs reveal more)
        stop = {
            # common words / chat noise
            "THIS", "THAT", "WHAT", "WHEN", "WHERE", "WHICH", "WHO",
            "HOW", "HELP", "NEED", "WANT", "WILL", "WITH", "FROM",
            "ABOUT", "THE", "AND", "FOR", "YOU", "YOUR", "ARE",

            # generic app terms
            "NEWS", "DATE", "TIME", "MOON", "WIND", "RAIN",

            # sports leagues / orgs
            "NBA", "WNBA", "NFL", "NHL", "MLB", "MLS", "UFC",
            "FIFA", "UEFA", "EPL", "NCAA", "NCAAF", "NCAAB",
            "IPL", "F1", "NBAA",

            # common sports abbreviations / event terms
            "MVP", "DPOY", "ROY", "GOAT", "OT", "PK", "VAR",
            "WC", "SB", "AFCCG", "NFCCG",
        }

        weak = {t for t in tokens if t not in stop}
    ...
    def _sanity_validate_intent(self, intent: str, ql: str, q_original: str) -> str:
        ...
        if intent == "crypto":
            ...
            # IMPORTANT: only accept symbols if user used $SYMBOL format (avoid uppercasing entire query)
            has_symbol = bool(re.search(r"\$[A-Z]{2,6}\b", q_original))
...
""".strip()
    ...
    async def search(self, query: str, retries: int = 3, backoff_factor: float = 0.5) -> list:
        ...
        forced = self._force_intent_from_terms(query_lower, query)
        if forced:
            try:
        ...
        query_type = self._sanity_validate_intent(query_type, query_lower, query)
