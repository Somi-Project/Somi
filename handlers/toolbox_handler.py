...
def create_tool_job(
    ...
    enforce_policy({"trust": trust, "action": "plan"})
    return JobsEngine().run_create_tool(
        name=name, description=description, mode=mode, active=active
    )


def dispatch_or_run(
    tool_name: str, args: dict | None = None, trust: str = "TRUSTED"
) -> dict:
    enforce_policy({"trust": trust, "action": "plan"})
    ctx = ToolContext(
