...
## Manual regression from user report (fixed)
- **Issue A (finance follow-up drift):** after "what's the price of bitcoin", follow-up "what was the price in october 2024" could fall back to irrelevant web results.
  - **Fix:** added finance context-symbol inference from last tool results in `agents.py` and pass it into `search_historical_price(...)` so BTC context is preserved.
- **Issue B (news follow-up headline fragment):** "expand on full support for US action in Iran" could trigger broad clarify with unrelated options.
  - **Fix:** added explicit phrase extraction + direct title/snippet substring resolution in `FollowUpResolver` before fuzzy tie-break logic.
- Regression simulation now shows:
  - BTC follow-up resolves via `yfinance_history` with `BTC-USD`.
  - Headline-fragment follow-up resolves directly to the intended URL.
