...
from gui.qt import QCheckBox, QComboBox, QDialog, QHBoxLayout, QLabel, QLineEdit, QMessageBox, QPushButton, QTimer, QVBoxLayout, Qt
...
def twitter_developer_tweet(app):
    ...
    def send_tweet():
        ...
        app.output_area.ensureCursorVisible()
        cmd = [sys.executable, "-m", "workshop.cli.somi", "devpost"]
        env = os.environ.copy()
...
def twitter_autotweet_toggle(app):
    ...
    if app.twitter_autotweet_process and app.twitter_autotweet_process.poll() is None:
        ...
        def start_autotweet():
            ...
            cmd = [sys.executable, "-m", "workshop.cli.somi", "aiautopost", "--name", agent_key] + (["--use-studies"] if use_studies else [])
            env = os.environ.copy()
            env["PYTHONUNBUFFERED"] = "1"
...
def twitter_autoresponse_toggle(app):
    ...
    if app.twitter_autoresponse_process and app.twitter_autoresponse_process.poll() is None:
        ...
        def start_autoresponse():
            ...
            cmd = [sys.executable, "-m", "workshop.cli.somi", "aiautoreply", "--name", agent_key, "--limit", str(limit)] + (["--use-studies"] if use_studies else [])
            env = os.environ.copy()
            env["PYTHONUNBUFFERED"] = "1"
...
def twitter_login(app):
    ...
    app.output_area.ensureCursorVisible()
    cmd = [sys.executable, "-m", "workshop.cli.somi", "gencookies"]
    env = os.environ.copy()
    env["PYTHONUNBUFFERED"] = "1"
    try:
        process = subprocess.Popen(
