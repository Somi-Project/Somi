...
from workshop.toolbox.stacks.research_core.role_overlay import resolve_role_context
...
class AgentpediaGrowthTask:
    ...
    def run(self, ctx: HeartbeatContext) -> list[dict[str, Any]]:
        ...
        try:
            grow_out = self.toolbox_runtime.run(
                "research.artifacts",
                {
                    "action": "agentpedia_grow",
                    "role": role,
                    "interests": interests,
                    "max_facts": max_facts,
                    "mode": "safe",
                },
                {"source": "heartbeat", "approved": True, "user_id": "default_user"},
            )
            if not bool((grow_out or {}).get("ok", False)):
                ctx.state.last_agentpedia_error = str((grow_out or {}).get("error") or "agentpedia_grow failed")
                return []
            result = dict((grow_out or {}).get("result") or {})

            stats_out = self.toolbox_runtime.run(
                "research.artifacts",
                {"action": "agentpedia_stats"},
                {"source": "heartbeat", "approved": True, "user_id": "default_user"},
            )
            if not bool((stats_out or {}).get("ok", False)):
                ctx.state.last_agentpedia_error = str((stats_out or {}).get("error") or "agentpedia_stats failed")
                stats: dict[str, Any] = {}
            else:
                stats = dict((stats_out or {}).get("result") or {})
